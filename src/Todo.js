...
function TodoElement(props) {
  ...
  function changeCheckedTodo() {
    if (checked == "") {
      ...
      changeChecked("");
      props.changedChecked((prev) =>
        prev.filter((item) => item !== props.index[1])
      );
    }
  }
  if (isTodoElementExist == true) {
    return (
      <li onClick={changeCheckedTodo} className={checked}>
        {props.text}{" "}
  ...
  return (
    <div>
      <div id="myDIV" class="header">
        ...
        <span
          onClick={() => {
            props.AddTodoElement([
              ...props.todoEls,
              <TodoElement
                checked={props.checked}
                checkedStatus=""
                text={text.current.value}
                index={[props.index, props.todoEls.length]}
                key={props.todoEls.length + 1}
                del={props.del}
                changedChecked={props.changedChecked}
              />,
            ]);
          }}
          className="addBtn"
