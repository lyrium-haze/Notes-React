...
function SaveBtn(props) {
  function save() {
    if (
      props.note.innerText == "" &&
      props.todoEls.length == 0 &&
    ...
    request.onsuccess = (event) => {
      ...
      request.onsuccess = () => {
        ...
        if (note == undefined) {
          note = {
            text: props.note.innerText,
            todoEls: todoArr,
            todoElsChecked: props.checked,
            title: props.title.innerText,
            subject: subject,
          };
          console.log(note);
          let a = db
...
function CloseBtn(props) {
  return (
    <button onClick={closeNote(props)} className="btn">
      Close
    </button>
  );
}
export default function Note(props) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < props.list.length; i++) {
      let checkedStatus = "";
      if (props.checked.includes(i)) checkedStatus = "checked";
      loadTodo.push(
        <TodoElement
    ...
    if (loadTodo.length > 0) addTodo("Delete");
    if (props.title != undefined && props.title != "") setTitleClass("title");
  ...
  function makeActive(e) {
    if (e.currentTarget.classList.contains("note_active")) return;
        ...
        <div className="btns-panel">
          ...
          {active == "note_active" ? (
            <DeleteBtn
              ...
              note={note.current}
              func={changeActive}
              deleteFunc={changeNoteStatus}
              showNotification={props.showNotification}
            />
          ) : null}
          {active == "note_active" ? (
            <CloseBtn closeFunc={changeActive} />
          ) : null}
          {active == "note_active" ? (
            <button
              onClick={() => {
                if (isTodoAdded == "Add") addTodo("Delete");
                else {
                  addTodo("Add");
                  AddTodoElement([]);
                }
              ...
              className="btn"
            >
              {isTodoAdded} To-Do list
            </button>
          ) : null}
          {active == "note_active" && titleClass == "title_hidden" ? (
            <button
              className="btn"
              onClick={() => {
                setTitleClass("title");
