...
function Con(props) {
  if (props.length < 1) return <Welcome />;
  else return <div></div>;
}
...
function App() {
  ...
  useEffect(() => {
    if (savedNotes.length > 0 || selectedValue != "") {
      const oldNotes = savedNotes
        ...
        .map((note) => (
          <Note
            list={note.value.todoEls}
            checked={note.value.todoElsChecked}
            key={note.key}
  ...
  useEffect(() => {
    if (selectedValue != "") {
      relevantNotes = allNotes.filter(
        (note) => note.value.subject == selectedValue
      );
      getNote(relevantNotes);
    }
  }, [selectedValue]);
